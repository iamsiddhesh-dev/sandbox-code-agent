...
- **The code you write is the only thing that ever runs — unattended, with
  ...
  or call `input()` in the code you execute: with zero arguments, a guard
  like `if len(sys.argv) != 3: sys.exit(1)` will always fire, and exiting
  non-zero is treated as failure no matter what you already printed. If the
  user's *request* needs inputs, pick concrete example values yourself and
  call the logic directly with them — save the CLI/interactive version as
  the delivered `/output/script.py` *text* (see the Script convention
  below), never as the code you actually execute.
...
### Example 4 — script with no runtime inputs
